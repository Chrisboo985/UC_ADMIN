...
// 用户列表视图主组件
export function RebaseOperationQueueView() {
  ...
  // 筛选条件是否能够重置
  const canReset = !!filtersForEdit.state.id;
  ...
  const getList = useCallback(async () => {
    ...
    await getRebaseRateQueueIndexAPI({ coin: 'KSN', ...params })
      .then((apiResult) => {
  ...
  const handleFormSubmit = async (formData: EventSchemaType) => {
    try {
      if (currentUser?.id) {
        // 编辑模式
        await updateRebaseRateQueueAPI({ coin: 'KSN', id: currentUser.id, ...formData });
        toast.success('修改成功');
      } else {
        // 新增模式
        await createRebaseRateQueueAPI({ coin: 'KSN', ...formData });
        toast.success('新增成功');
      }
