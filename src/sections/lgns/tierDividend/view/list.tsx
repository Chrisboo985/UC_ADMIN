...
import {
  ...
  getStakeIndexAPI,
  getTeamBonusRateIndexAPI,
  updateCommunityBonusRateAPI,
  updateTeamBonusRateAPI,
...
// 用户列表视图主组件
export function TierDividendView() {
  ...
  const comfirmEditRate = useCallback(
    async (rate: number) => {
      ...
      try {
        await updateTeamBonusRateAPI({ id: editDialog.id, rate }).then((res) => {
          if (res.code === 0) {
            toast.success('修改成功');
            getList();
            toast.dismiss(stop);
  ...
  const getList = useCallback(async () => {
    try {
      ...
      const response = await getTeamBonusRateIndexAPI();
      if (response.code === 0) {
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      minWidth: 100,
      flex: 1,
      renderCell: (params) => <CellWithTooltipCopy value={params.row.weights} />,
    },
    {
      field: 'rate',
      headerName: '分红比例',
