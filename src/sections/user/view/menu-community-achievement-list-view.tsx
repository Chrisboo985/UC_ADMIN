...
import { type RechargeListItem, type RechargeListParams } from './menu-community-achievement-list-view.types'
...
// 用户列表视图主组件
export function MenuCommunityAchievementListView(props: { h: boolean }) {
  ...
  const PAGINATION_DEFAULT_CONFIG = React.useCallback(
    () => ({
      page: 1,
      pageSize: 10,
      total: 0,
  ...
  const paginationConfig = useSetState(PAGINATION_DEFAULT_CONFIG())

  // 过滤数据
  const [filteredData, setFilteredData] = useState<RechargeListItem[]>([]);
  const tableRows = React.useMemo(
    () => {
      const { page, pageSize } = paginationConfig.state
      const startPosition = (page - 1) * pageSize

      return filteredData.slice(startPosition, startPosition + pageSize)
    },
    [filteredData, paginationConfig.state]
  )
  // 总计
  // 当前选中ID
  ...
  // 筛选条件是否能够重置
  const canReset = !!params.state.community_address || !!params.state.created_at_start || !!params.state.created_at_end;

  React.useEffect(
    () => {
      if (!params.state.community_address) {
        setFilteredData([])
        paginationConfig.setState(PAGINATION_DEFAULT_CONFIG())
      }
    },
    [params.state.community_address]
  )

  // 获取用户列表
  const getList = useCallback(async () => {
    if (!params.state.community_address) return
    setUsersLoading(true);

    getMenuCommunityAchievementList(params.state)
      .then((apiResult) => {
        ...
        if (code === 0) {
          // 如果为空，需要设置默认值
          setFilteredData((data?.list || []).map((item, index) => ({ ...item, id: index })));
          paginationConfig.setState({ total: data?.total || 0, page: 1 })
        } else {
          toast.error(apiResult.message);
          setFilteredData([]);
          paginationConfig.setState({ total: 0, page: 1 })
        }
      })
      .finally(() => {
  ...
  const handleFilterData = () => {
    ...
    const { community_address, created_at_start, created_at_end } = params.state
  ...
  const handleFilterAddress = useCallback(
    (event: React.ChangeEvent<HTMLInputElement>) => {
      params.setState({ community_address: event.target.value });
    },
  ...
  const CustomToolbarCallback = useCallback(
    () => (
      <CustomToolbar
        ...
        filteredResults={tableRows.length}
      />
    ),
    [
      canReset,
        ...
        >
          ...
          >
            <FormControl component="fieldset" sx={{ flexShrink: 1, minWidth: { xs: 1, md: 200 } }}>
              <TextField
                fullWidth
                value={params.state.community_address}
                onChange={handleFilterAddress}
                placeholder="请输入0号社区地址"
                InputProps={{
                  startAdornment: (
          ...
          <DataGrid
            ...
            onPaginationModelChange={(model) => {
              ...
              paginationConfig.setState({ page: model.page + 1, pageSize: model.pageSize })
            }}
            ...
            rowCount={paginationConfig.state.total}
            paginationModel={{ page: paginationConfig.state.page - 1, pageSize: paginationConfig.state.pageSize }}
...
export function UserTableFiltersResult({ filters, totalResults, sx }: Props) {
  const handleRemoveAddress = useCallback(() => {
    filters.setState({ community_address: '' });
  }, [filters]);
  ...
  const handleReset = useCallback(() => {
    filters.setState({ community_address: '', created_at_start: null, created_at_end: null });
  }, [filters]);

  return (
    <FiltersResult totalResults={totalResults} onReset={handleReset} sx={sx}>
    <FiltersBlock label="地址:" isShow={!!filters.state.community_address}>
      <Chip {...chipProps} label={filters.state.community_address} onDelete={handleRemoveAddress} />
    </FiltersBlock>
      <FiltersBlock label="时间:" isShow={!!filters.state.created_at_start && !!filters.state.created_at_end }>
        <Chip
          {...chipProps}
