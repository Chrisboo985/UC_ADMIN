...
export function NodeOrderListView(props: { h: boolean }) {
  ...
  type QueryType = 'network' | 'community';
  ...
  const filtersForEdit = useSetState<{
    ...
    created_at_end?: number;
    created_at_start?: number;
    product_id?: number | typeof Infinity;
    type?: QueryType;
  }>({
    member_address: '',
    created_at_end: undefined,
  ...
  const handleTypeFilter = React.useCallback(
    (t: QueryType) => {
      filtersForEdit.setState({ type: t });
    },
    [filtersForEdit]
  )
  ...
  const getList = useCallback(async () => {
    // 若未填写地址，不请求接口，清空数据
    if (!filters.state.member_address || !filters.state.member_address.trim()) {
      setFilteredData([]);
      setTotalCount(0);
      setTotalPurchase(0);
      return;
    ...
    if (filters.state.product_id && Number.isFinite(filters.state.product_id)) {
      params.product_id = filters.state.product_id;
    }
    if (filters.state.created_at_start != null) params.created_at_start = filters.state.created_at_start;
    if (filters.state.created_at_end != null) params.created_at_end = filters.state.created_at_end;

    setUsersLoading(true);
    await axiosForApi.post('/admin/order/network_list', params)
      .then((res) => {
        const { data, code, message } = res as any;
        if (code === 0) {
  ...
  const handleFilterData = () => {
    filters.setState({
      ...
      product_id: filtersForEdit.state.product_id === Infinity ? undefined : filtersForEdit.state.product_id,
      page: 1,
      page_size: pagination.pageSize,
      type: (filtersForEdit.state.type || 'network'),
    });
  };
      ...
      <DashboardContent sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        >
          ...
          >
            ...
            <FormControl component="fieldset" sx={{ flexShrink: 1, minWidth: { xs: 1, md: 200 } }}>
              <InputLabel id="query-type-select-label">查询类型</InputLabel>
              <Select
                labelId="query-type-select-label"
                id="query-type-select"
                value={filtersForEdit.state.type}
                label="查询类型"
                onChange={(e) => handleTypeFilter(e.target.value as 'network' | 'community')}
              >
                <MenuItem value={'network'}>网体业绩</MenuItem>
                <MenuItem value={'community'}>社区业绩</MenuItem>
              </Select>
            </FormControl>
            ...
            <FormControl component="fieldset" sx={{ flexShrink: 1, minWidth: { xs: 1, md: 200 } }}>
              <Button variant="contained" onClick={handleFilterData} disabled={!filtersForEdit.state.member_address}>
                查询
              </Button>
            </FormControl>
          </Stack>
