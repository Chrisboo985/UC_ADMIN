...
export const FormSchema = zod.object({
  remove_amount: zod.string().refine(
    (val) => {
...
type Props = {
  onClose: () => void;
  onSuccess: (data: { member_id: number; remove_amount: number }) => void;
  currentEvent: IUserItemforlist;
};

export function ReduceTurbineForm({ currentEvent, onClose, onSuccess }: Props) {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      onSuccess({
        ...
        remove_amount: Number(data.remove_amount),
      });
      reset();
      onClose();
    } catch (error) {
