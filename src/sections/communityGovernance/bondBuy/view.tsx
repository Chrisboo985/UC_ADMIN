...
class NodeManager {
  ...
  async fetchNode(params: any): Promise<any> {
    ...
    const promise = getMemberCommunityBondSubAPI(params).then((response) => {
...
export function BondBuyView() {
  ...
  async function getRootNode() {
    try {
      const res = await getMemberCommunityBondSubAPI({
        page: 1,
        page_size: 999,
  ...
  const handleSearchAddress = useCallback(async () => {
    ...
    const result = await Promise.allSettled([getMemberCommunityBondSubAPI(params)]);
    setIsLoading(false);
    let node: any;
