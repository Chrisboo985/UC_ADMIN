...
class NodeManager {
  ...
  async fetchNode(params: any): Promise<any> {
    ...
    const promise = getCommunityBondWithKspSubAPI(params).then((response) => {
...
export function CommunityPerformanceSummaryView() {
  ...
  async function getRootNode() {
    try {
      const res = await getCommunityBondWithKspSubAPI({
        page: 1,
        page_size: 999,
  ...
  const handleSearchAddress = useCallback(async () => {
    ...
    const result = await Promise.allSettled([getCommunityBondWithKspSubAPI(params)]);
    setIsLoading(false);
    let node: any;
