...
// 获取当天的 0 时0秒的时间戳
const getTodayStartTimestamp = () => dayjs().startOf('day').valueOf();
...
class NodeManager {
  ...
  async fetchNode(params: any): Promise<any> {
    ...
    const promise = getMemberCommunityStakeSubAPI(params).then((response) => {
...
export function CommunityStakingView() {
  ...
  const filtersForEdit = useSetState({ date: getTodayStartTimestamp(), id: '', address: '' });
  ...
  const fetchNodes = useCallback(
    async function _fetchNodes(parentId: number) {
      ...
      try {
        const params = {
          date_timestamp: filtersForEdit.state.date / 1000,
    ...
    [nodeManager, filtersForEdit.state.date]
  );
  ...
  async function getRootNode() {
    try {
      const res = await getMemberCommunityStakeSubAPI({
        date_timestamp: filtersForEdit.state.date / 1000,
        page: 1,
        page_size: 999,
  ...
  const handleSearchAddress   = useCallback(async() =>  {
    ...
    const result = await Promise.allSettled([getMemberCommunityStakeSubAPI(params)]);
    setIsLoading(false);
    let node: any;
  ...
  }, [filtersForEdit.state.date, filtersForEdit.state.address]);
    ...
    <DashboardContent sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
      ...
      >
        <FormControl sx={{ flex: 1, maxWidth: 300 }}>
          <DatePicker
            label="选择日期"
            value={dayjs(filtersForEdit.state.date)}
            onChange={(date) => {
              filtersForEdit.setState({
                date: date ? date.valueOf() : 0,
              });
              handleSearchAddress();
              resetCanvas();
            }}
